...
Atmosphere::Atmosphere(int num_levels, const ColumnView temp,
                       ...
                       const ColumnView could_f, const ColumnView uv_ice_nuc,
                       Real pblh)
    : temperature(temp), pressure(press), vapor_mixing_ratio(qv), height(ht),
      hydrostatic_dp(pdel), cloud_fraction(could_f),
  ...
  EKAT_REQUIRE_MSG(could_f.extent(0) >= num_levels + 1,
                   "could fraction must have extent >= " << num_levels);
  EKAT_REQUIRE_MSG(
