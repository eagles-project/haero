...
struct FloatingPoint<ekat::Pack<ScalarType,HAERO_PACK_SIZE>> {
  ...
  KOKKOS_INLINE_FUNCTION
  static bool zero(const ekat::Pack<ScalarType,HAERO_PACK_SIZE>& x, const Real tol = zero_tol) {
    EKAT_KERNEL_ASSERT(tol > 0);
  ...
  KOKKOS_INLINE_FUNCTION
  static PackType safe_denominator(const ekat::Pack<ScalarType,HAERO_PACK_SIZE>& x,
                                   const Real tol = zero_tol) {
    EKAT_KERNEL_ASSERT(tol > 0);
