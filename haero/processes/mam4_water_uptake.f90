...
use modal_aero_microp_precision, only: dp
...
public :: makoh_cubic, makoh_quartic
   ...
   do m = 1, nmodes
      ...
      do k = top_lev, pver
         do i = 1, ncol
            ...
            if (maer(i,k,m) .gt. 1.0e-31_wp) then
   ...
   do m = 1, nmodes
      ...
      do k = str_lev, end_lev            ! make this change so that the foloowing subroutine can be callable for a grid cell
         do i = 1, ncol
            ...
            call modal_aero_kohler(use_bisection, dryrad(i:i,k,m), &
...
!-----------------------------------------------------------------------
      subroutine modal_aero_kohler(   &
          use_bisection, rdry_in, hygro, s, rwet_out, im )
...
!             find smallest real(wp) solution
              ...
              do n=1,4
                 ...
                 if(xr.gt.r(i)) cycle
                 if(xr.lt.rdry(i)*(1._wp-eps)) cycle
                 if(xr.ne.xr) cycle
                 r(i)=xr
                 nsol=n
              end do
              if(nsol.eq.0)then
                 write(iulog,*)   &
                  'ccm kohlerc - no real(wp) solution found (quartic)'
...
!                find smallest real(wp) solution
                 ...
                 if(nsol.eq.0)then
                    write(iulog,*)   &
                     'ccm kohlerc - no real(wp) solution found (cubic)'
...
!
      ...
      complex(wp) :: cb(imx), cb0(imx), cb1(imx),   &
...
!----------------------------------------------------------------------
   ...
   real(wp), parameter :: toler = 10._wp * epsilon(1._wp)      ! tolerance for bisection
   ...
   do
      ...
      if (abs(left-right)/middle < toler) then
         rw = middle
         exit
      else if (f_middle .eq. 0._wp) then
         rw = middle
         exit
