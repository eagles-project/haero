...
subroutine ternary_nuc_merik2007( t, rh, c2, c3, j_log, ntot, nacid, namm, r )
  ...
  if (t_onset > t) then
    ...
    ntot = 57.40091052369212_wp - 0.2996341884645408_wp*t + &
      ...
      0.005704394549007816_wp*log(c3)**3 + 3.4098703903474368_wp*log(j) - &
      0.014916956508210809_wp*t*log(j) + 0.08459090011666293_wp*log(c3)*log(j) - &
      0.00014800625143907616_wp*t*log(c3)*log(j) + 0.00503804694656905_wp*log(j)**2
    ...
    r = 3.2888553966535506e-10_wp - 3.374171768439839e-12_wp*t + &
      ...
      4.141077193427042e-15_wp*log(j) - 2.6813110884009767e-14_wp*t*log(j) + &
      1.2879071621313094e-12_wp*log(c3)*log(j) - &
      3.80352446061867e-15_wp*t*log(c3)*log(j) - 1.8790172502456827e-14_wp*log(j)**2
    ...
    nacid = -4.7154180661803595_wp + 0.13436423483953885_wp*t - &
      ...
      0.00040072788891745513_wp*t*log(c3)**3 + 1.3276469271073974_wp*log(j) - &
      0.006167654171986281_wp*t*log(j) - 0.11061390967822708_wp*log(c3)*log(j) + &
      0.0004367575329273496_wp*t*log(c3)*log(j) + 0.000916366357266258_wp*log(j)**2
    ...
    namm = 71.20073903979772_wp - 0.8409600103431923_wp*t + &
      ...
      0.00009377332742098946_wp*log(c3)**3 + 0.5202974341687757_wp*log(j) - &
      0.002419872323052805_wp*t*log(j) + 0.07916392322884074_wp*log(c3)*log(j) - &
      0.0003021586030317366_wp*t*log(c3)*log(j) + 0.0046977006608603395_wp*log(j)**2
