...
!> mam_rename_subarea Fortran module.
module mam_rename_subarea_bridge

  implicit none
  private

  ! Module functions
  public :: mam_rename_subarea_init, &
            run_bridge, &
            mam_rename_subarea_finalize, &
            compute_diameter_bridge

contains

! Initializes the prognostic process.
subroutine mam_rename_subarea_init() bind(c)
  use haero, only: model
  use mam_rename_subarea, only: init
  implicit none
...
subroutine run_bridge(t, dt, progs, atm, diags, tends) bind(c)
  ...
  use mam_rename_subarea, only: run
  implicit none
...
subroutine mam_rename_subarea_finalize() bind(c)
  use haero, only: model
  use mam_rename_subarea, only: finalize
  implicit none

  call finalize(model)
end subroutine mam_rename_subarea_finalize
