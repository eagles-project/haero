...
void ModalAerosolConfig::index_modal_species(const std::map<std::string, std::vector<std::string> >& mode_species) {
  ...
  for (auto iter = mode_species.begin(); iter != mode_species.end(); ++iter) {
    ...
    for (int s = 0; s < aero_species.size(); ++s) {
      int species_index = -1;
      for (unsigned i=0; i<h_aerosol_species.size() && species_index == -1; ++i)
...
ModalAerosolConfig create_simple_test_config() {
  ...
  const std::vector<std::pair<Real,Real>> mode_min_max_diams = {std::make_pair(1e-8, 1e-7), std::make_pair(1e-7, 1e-6)};
  ...
  for (int m=0; m<nmodes; ++m) {
    test_modes[m] = Mode(mode_names[m], mode_min_max_diams[m].first, mode_min_max_diams[m].second,
  ...
  for (int s=0; s<nspec; ++s) {
    aeros[s] = AerosolSpecies(spec_names[s], spec_symbs[s], "test_aerosol", spec_molec_weights[s],
      spec_dry_radius[s], spec_dens[s], spec_hygro[s]);
    mode_spec_map[mode_names[1]].push_back(spec_symbs[s]);
  }
  mode_spec_map[mode_names[0]].push_back(spec_symbs[0]);

  const std::vector<GasSpecies> test_gases = {GasSpecies("test_gas0", "TG0", "test_gas", g_to_kg*40)};
