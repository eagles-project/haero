...
struct ModeWetRadius {
  ...
  void operator()(const int pack_idx) const {
    ...
    wet_radius_meters(pack_idx).set(use_dry_radius,
    ...
    const PackType dry_vol = mode.mean_particle_volume_from_diameter(
        2 * modal_dry_radius_meters(pack_idx));
    PackType wet_vol = mode.mean_particle_volume_from_diameter(
        2 * wet_radius_meters(pack_idx));
    EKAT_KERNEL_ASSERT((wet_vol >= dry_vol).all());
    const PackType water_vol =
        (wet_vol - dry_vol) *
