...
/** @brief This struct computes a reduction across the species in a mode to find the
  modal mean particle volume.

  The mean particle volume is proportional to the 3rd moment of the mode's log-normal PDF.
*/
struct ModalMeanParticleVolume {
  ...
  void operator() (const int pack_idx) const {
    ...
    for (int i=0; i<n_aerosols_in_mode; ++i) {
      ...
      vol_mixing_ratio += mass_mixing_ratios(s, pack_idx) / aerosols_in_mode(i).density;
    }
    modal_mean_volume(pack_idx) = vol_mixing_ratio / number_mixing_ratios(mode_idx, pack_idx);
  }
  ...
  void operator() (const typename DeviceType::MemberType& column_team) const {
    ...
    Kokkos::parallel_reduce(Kokkos::TeamThreadRange(column_team, n_aerosols_in_mode),
        [=] (const int& i, PackType& update) {
          const int s = population_indices(i);
          update += mass_mixing_ratios(s, pack_idx) / aerosols_in_mode(i).density;
        }, vol_mixing_ratio);
    modal_mean_volume(pack_idx) = vol_mixing_ratio / number_mixing_ratios(mode_idx, pack_idx);
  }
};
