...
struct ModalHygroscopicity {
  ...
  void operator() (const int pack_idx) const {
    ...
    for (int i=0; i<n_aerosols_in_mode; ++i) {
      ...
      hyg += mass_mixing_ratios(s,pack_idx) * aerosols_in_mode(i).hygroscopicity / aerosols_in_mode(i).density;
      vol_mix_ratio += mass_mixing_ratios(s, pack_idx) / aerosols_in_mode(i).density;
    }
  ...
  void operator() (const typename DeviceType::MemberType& column_team) const {
    ...
    PackType hyg(0);
    PackType vol_mix_ratio(0);
    const auto thread_policy = Kokkos::TeamThreadRange(column_team, n_aerosols_in_mode);
    Kokkos::parallel_reduce(thread_policy, [=] (const int i, PackType& update) {
        const int s = population_indices(i);
        update += mass_mixing_ratios(s, pack_idx) * aerosols_in_mode(i).hygroscopicity / aerosols_in_mode(i).density;
      }, hyg);
    Kokkos::parallel_reduce(thread_policy, [=] (const int i, PackType& update) {
        const int s = population_indices(i);
