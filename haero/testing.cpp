...
class ColumnPool {
 ...
 public:
  ...
  ColumnPool(size_t num_vertical_levels, size_t initial_num_columns = 64)
    : num_levels_(num_vertical_levels),
      num_cols_(initial_num_columns),
      col_in_use_(initial_num_columns, 0),
      memory_(reinterpret_cast<Real*>(
        Kokkos::kokkos_malloc("Column pool",
  ...
  ColumnView column_view() {
    ...
    if (i == num_cols_) { // all columns in the pool are in use!
      ...
      memory_ = reinterpret_cast<Real*>(Kokkos::kokkos_realloc(memory_, sizeof(Real) * num_levels_ * num_cols_));
    }
...
ColumnView create_column_view(int num_levels) {
  ...
  if (iter == pools_.end()) {
    auto result = pools_.emplace(num_levels,
                                 std::unique_ptr<ColumnPool>(new ColumnPool(num_levels)));
    iter = result.first;
  }
