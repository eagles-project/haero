...
Reaction::Reaction(std::string mtype_str,
  ...
  {
    type = arrhenius;
    rate_coefficients["A"]  = (mrate_coefficients.find("A")
                               != mrate_coefficients.end()) ?
                              mrate_coefficients["A"] : 1.0;
    rate_coefficients["Ea"] = (mrate_coefficients.find("Ea")
                               != mrate_coefficients.end()) ?
                              mrate_coefficients["Ea"] : 0.0;
    rate_coefficients["B"]  = (mrate_coefficients.find("B")
                               != mrate_coefficients.end()) ?
                              mrate_coefficients["B"] : 0.0;
    rate_coefficients["D"]  = (mrate_coefficients.find("D")
                               != mrate_coefficients.end()) ?
                              mrate_coefficients["D"] : 300.0;
    rate_coefficients["E"]  = (mrate_coefficients.find("E")
                               != mrate_coefficients.end()) ?
                              mrate_coefficients["E"] : 0.0;
  }else if (type_str.compare("troe") == 0)
  {
    type = troe;
    rate_coefficients["k0_A"]   = (mrate_coefficients.find("k0_A")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["k0_A"] : 1.0;
    rate_coefficients["k0_B"]   = (mrate_coefficients.find("k0_B")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["k0_B"] : 0.0;
    rate_coefficients["k0_C"]   = (mrate_coefficients.find("k0_C")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["k0_C"] : 0.0;
    rate_coefficients["kinf_A"] = (mrate_coefficients.find("kinf_A")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["kinf_A"] : 1.0;
    rate_coefficients["kinf_B"] = (mrate_coefficients.find("kinf_B")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["kinf_B"] : 0.0;
    rate_coefficients["kinf_C"] = (mrate_coefficients.find("kinf_C")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["kinf_C"] : 0.0;
    rate_coefficients["Fc"]     = (mrate_coefficients.find("Fc")
                               != mrate_coefficients.end()) ?
                                  mrate_coefficients["Fc"] : 0.6;
    rate_coefficients["N"]      = (mrate_coefficients.find("N")
                               != mrate_coefficients.end()) ?
...
void ChemSolver::parse_tchem_inputs(SimulationInput& sim_inp)
{
  ...
  try {
    ...
    if (root["tchem_inputs"] and root["tchem_inputs"].IsMap()) {
      ...
      if (not node["detail"])
      {
        throw YamlException("problem specific entry has no detail boolean (detail).");
      }
      else if (not node["nbatch"])
      {
        throw YamlException("problem specific entry does not specify number "
                            "of batches (nbatch).");
      }
      else if (not node["verbose"])
      {
        throw YamlException("problem specific entry has no verbose boolean (verbose).");
      }
      else
      {
...
void SourceTermToyProblem_TemplateRun( /// input
  ...
  const RealType2DViewType& kfor,
  const RealType2DViewType& krev,
