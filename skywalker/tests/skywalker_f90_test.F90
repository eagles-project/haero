...
program skywalker_f90_test
  ...
  character(len=255)  :: input_file = "f90_test.yaml"
  ...
  ensemble = load_ensemble("mam4", trim(input_file), "mam_box_model")

  ! Make sure everything is as it should be.

  ! process section
  assert(trim(ensemble%program_name) == "mam_box_model")
  ...
  assert(ensemble%num_modes == 4)
  assert(ensemble%num_populations == 21) ! all species in all modes
  assert(ensemble%num_gases == 13)

  ! ensemble information
  assert(ensemble%size == 3300)
  do i = 1, ensemble%size
    input = ensemble%inputs(i)

    ! Time stepping parameters
    assert(input%dt == 1._wp)
    assert(input%total_time == 1._wp)

    ! Atmospheric parameters
    assert(approx_equal(input%pressure, 100000._wp))
    assert(approx_equal(input%vapor_mixing_ratio, 0.4_wp))
    assert(approx_equal(input%height, 3000._wp))
    assert(approx_equal(input%hydrostatic_dp, 200._wp))
    assert(approx_equal(input%planetary_boundary_layer_height, 1100._wp))
  ...
  call ensemble%write_py_module("f90_test.py")
